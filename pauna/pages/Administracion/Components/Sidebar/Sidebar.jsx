...
export default function Sidebar({ isOpen, toggleSidebar }) {
  ...
  return (
    <>
      <div
        className={`bg-[#4333F9] fixed top-0 left-0 pt-10 rounded-r-lg duration-300 ${
          isOpen ? "w-80" : "w-20"
        }`}
      >
        <div
          className={`w-10 h-10 bg-white border border-[#4333F9] rounded-full absolute -right-5 top-3 flex items-center 
                justify-center
                $`}
        ...
        <div className="text-center text-black text-2xl font-semibold">
          <div className="flex items-center">
            ...
            <div className={`text-white ml-3 ${!isOpen && "hidden"}`}>
              PAUNA
            </div>
          </div>
        </div>
        <div className="pt-20">
          <Link href="/Administracion/Components/User/userWindow">
            <div
              className={`hover:bg-white cursor-pointer py-10 mb-2 flex items-center ${
                active === 1 && "bg-white"
              }`}
              onClick={() => setActive(1)}
            >
              <p
                className={`text-left text-black text-2xl font-semibold mx-5 ${
                  !isOpen && "hidden"
                }`}
              >
                Usuarios
