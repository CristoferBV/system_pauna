...
const getAllAdministradores = async (req, res) => {
    ...
    const [result] = await pool.query("SELECT u.UO_primer_nombre, u.UO_primer_apellido, u.UO_segundo_apellido, u.UO_identificador, ce.CE_correoElectronico, u.UO_contrasena, r.RL_descripcion FROM `pau_gnl_usuario`u INNER JOIN `pau_gnl_tbl_correoelectronico` ce ON u.UO_identificador_correo = ce.CE_idCorreo INNER JOIN `pau_gnl_rol` r ON u.UO_identificador_rol = r.RL_identificador WHERE r.RL_nombre = 'AdminBiblioteca'");
    console.log(result)
